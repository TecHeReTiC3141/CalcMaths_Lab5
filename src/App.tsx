...
export default function App() {
  ...
  const isDataValid = useMemo(() => {
    ...
    return intervalLeft < intervalRight && intervalLeft < interpolationPoint
       && interpolationPoint < intervalRight
       && currentFunction && nodesCount === Math.round(nodesCount)
  }, [inputVariant, intervalLeft, intervalRight, currentFunction, pointsData, interpolationPoint, nodesCount])
          ...
          <div className="flex gap-x-3 items-start">
            <div className="section">
              <div className="flex gap-x-1 items-start w-fit pb-1 border-b border-orange-400">
                <button className={clsx("text-sm text-gray-800 hover:bg-orange-200 p-2 rounded", inputVariant === InputVariant.Points && 'bg-orange-300')} 
                  onClick={() => setInputVariant(InputVariant.Points)}>Points</button>
                <button className={clsx("text-sm text-gray-800 hover:bg-orange-200 p-2 rounded", inputVariant === InputVariant.Function && 'bg-orange-300')} 
                  onClick={() => setInputVariant(InputVariant.Function)}>Function</button>
              </div>
              ...
              (
                <div className="flex gap-x-3 items-start">
                  <div className="flex flex-col gap-y-3">
                    ...
                    <input type="number" className="field" name="interpolation-point" value={interpolationPoint} onChange={(event) => setInterpolationPoint(+event.target.value)} />
  ...
  xn yn"/>
                  ...
                  <InputUploader setInterpolationPoint={setInterpolationPoint} setPoints={setPointsData} />
                </div>
              )
               : (
                <div className="flex flex-col gap-y-2">
                    <h4 className="font-bold text-lg">Function</h4>
                    <select value={currentFunction} onChange={(event) => setCurrentFunction(event.target.value)}>
                    ...
                    <input type="number" min="2" step="1" pattern="\d+" className="field" name="interpolation-point" value={nodesCount} onChange={(event) => setNodesCount(+event.target.value)} />
                    <h4 className="font-bold text-lg">Interval</h4>
                    <div className="flex gap-x-2 items-center">
                      <input type="number" className="field" name="interpolation-point" value={intervalLeft} onChange={(event) => setIntervalLeft(+event.target.value)} />
                      <p>:</p>
                      <input type="number" className="field" name="interpolation-point" value={intervalRight} onChange={(event) => setIntervalRight(+event.target.value)} />
                    </div>
            ...
            <button
              ...
              onClick={() => {
                ...
                if (inputVariant === InputVariant.Points) {
                  ...
                  const inputFunction = predefinedFunctions.find(({ label }) => label === currentFunction)!.equation
                  points = Array.from({ length: nodesCount + 1 }).map((_, index) => {
